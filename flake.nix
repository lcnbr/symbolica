    ...
    in {
      devShells = eachSystem (pkgs:
        let
          ...
          tools = [ pkgs.cargo pkgs.git pkgs.gnum4 pkgs.maturin pkgs.pkg-config pkgs.python312 pkgs.rustc pkgs.uv ]
      ...
      apps = eachSystem (pkgs:
        let
          libs = [ pkgs.gmp pkgs.mpfr ];
          path = [ pkgs.cargo pkgs.coreutils pkgs.git pkgs.gnum4 pkgs.maturin pkgs.pkg-config pkgs.python312 pkgs.rustc pkgs.tailscale pkgs.uv ]
            ++ libs ++ pkgs.lib.optionals pkgs.stdenv.isLinux [ pkgs.patchelf ];
          env = ''
            export GMP_MPFR_SYS_USE_SYSTEM_LIBS=1
            export LD_LIBRARY_PATH="${pkgs.lib.makeLibraryPath libs}:''${LD_LIBRARY_PATH:-}"
